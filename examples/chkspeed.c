...
int main(int argc, char *argv[])
{
	...
	if (CURLE_OK == res) {
		...
		res =
		    curl_easy_getinfo(curl_handle, CURLINFO_SIZE_DOWNLOAD,
				      &val);
		...
		res =
		    curl_easy_getinfo(curl_handle, CURLINFO_SPEED_DOWNLOAD,
				      &val);
		if ((CURLE_OK == res) && (val > 0))
